...
def _merge_halves(vals, wts, buf_v, buf_w, lo, mid, hi):
    """Merge two sorted runs [lo, mid) and [mid, hi). Only copies left half to buffer."""
    left_size = mid - lo
...
def _compute_weighted_median_single(y, w, left, right):
    ...
    return _compute_weighted_median(y, w, left, right, tmp_vals, tmp_wts, buf_v, buf_w)
...
class RangeMedian:
    ...
    def _get_mu_dist(self, left, right):
        ...
        if result is None:
            ...
            result = _compute_weighted_median_single(self._y, self._w, left, right)
